#!/bin/bash
# Hybrid ResNet mode - ResNet embedding + transformer blocks (WITHOUT positional embeddings)
goal=YourGoal
...
# Use ResNet-18 in HYBRID mode (ResNet embedding + transformer blocks, NO pos embeddings)
python run_joint_mm_rec.py \
    --config configs_joint_fedcola_coco_mm_img_txt.yml \
    --data_path $root \
    --exp_name FedCola_Joint_Coco_MM_ResNet18_Hybrid_NoPosEmbed \
    --shared_param attn \
    ...
    --split_type diri \
    --cncntrtn $cncntrtn \
    --model_name mome_resnet18_small_nopos \
    --resize 224 \
    --imnorm \
    --algorithm fedavg \
    --eval_type global \
